...
从 `assets/templates/tianqiaoxing/encoder_qei/` 选择性复制 QEI 驱动。若集成 OLED 菜单，则以 `oled_menu/` 为 UI 起点，再加入编码器模块。

所需 SysConfig：TIMG8 QEI + 编码器按键 GPIO；只有运行时确实需要周期轮询时才加入定时 tick。
