...
- Board shared-bus baseline: **Software I2C on PA0(SDA) / PA1(SCL)** — 与 OLED 共享同一 I2C 总线
- Packaged `imu_lsm6ds3` template: **Software I2C on PA28(SDA) / PA27(SCL)** — 独立的实测接线变体，不可与 PA0/PA1 配置混用
- I2C Address: 0x6A (SA0 to GND)
- Fusion algorithm: AHRS (Mahony) — pitch/yaw/roll output

## Template

从 `assets/templates/tianqiaoxing/imu_lsm6ds3/` 选择性复制驱动与融合层，并按目标接线修改 `.syscfg` 和 `myiic` 引脚宏。
