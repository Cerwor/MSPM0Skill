...
天猛星 MSPM0G3507 的 SPI 总线使用，重点覆盖板载 SPI1 共享总线与片选管理。

## Contents

- SPI 实例、引脚和占用关系
- W25Q64/LCD 共享总线
- SysConfig、DriverLib 与验证模式
...
## OLED 128×64 (I2C) 字体设计指南

天猛星 I2C0（PA0/SDA, PA1/SCL）可外接 0.96 寸 128×64 OLED 模块。**所有 128×64 OLED 项目必须使用 6×8 字体作为默认，禁止用 8×16 作为正文字体。**

### 为什么 8×16 是错的

| 字体 | 单字高度 | 128×64 屏可见行数 | 效果 |
|------|---------|------------------|------|
| \`OLED_6X8\` | 8px | **8 行**，信息密度合理 | ✅ 正常 UI |
| \`OLED_8X16\` | 16px | **4 行**，一个字符占屏幕 25% | ❌ 字体巨大、丑陋 |

> **铁律**：8×16 仅可用于屏幕标题（如 \`OLED_DrawString(0,0,"STATUS",OLED_8X16)\`），正文、标签、状态值一律用 6×8。

### 128×64 推荐布局

\`\`\`c
// 标题区（8×16，1 行）
OLED_DrawString(0, 0, "KEY STATUS", OLED_8X16);

// 内容区（6×8，最多 7 行）
OLED_DrawString(0, 20, "BTN1: PRESSED",  OLED_6X8);
OLED_DrawString(0, 32, "BTN2: RELEASED", OLED_6X8);
OLED_DrawString(0, 44, "UPTIME: 12s",    OLED_6X8);
\`\`\`
...
| OLED 引脚 | 天猛星引脚 | 说明 |
|----------|-----------|------|
| SDA | PA0 | I2C0 SDA，板上 2.2kΩ 上拉 |
| SCL | PA1 | I2C0 SCL，板上 2.2kΩ 上拉 |
| VCC | 3.3V | — |
| GND | GND | — |
