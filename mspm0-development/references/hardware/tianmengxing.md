...
| PA8、PA9；PB4、PB5 | UART1 器件复用候选，不是已验证板级默认；恢复第二路 UART 前核对板卡版本原理图和 Flash 共存 |
...
PB6～PB9 禁止作为通用 UART、PWM 或 GPIO 模板默认值。尤其是 PB6/PB7，同时启用 UART 会干扰 Flash 片选/数据线并可能造成输出争用。
