...
- The Tianqiaoxing source contributes distilled MSPM0G3519 common-peripheral facts and a minimal GPIO smoke-test template.
...
| `mspm0kit-tianqiaoxing` | Device/package baseline, common GPIO/ADC/PWM/Timer/UART/SPI/I2C patterns, and the minimal blink source/config | Board-application references, application middleware, composition helpers, and application templates are deliberately not retained. Supply current task material if one of those applications is needed later. |
...
3. For Tianqiaoxing common peripherals, use the user's exact project and current schematic first, then the bounded common-peripheral references. The skill intentionally carries no advanced board-application wiring.
...
- Full maintained Tianmengxing routing plus bounded Tianqiaoxing routing for the common peripherals documented in `references/hardware/`.
